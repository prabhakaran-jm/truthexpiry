...
def test_fallback_refs_are_capped():
    hits = tuple(_hit(f"body-{index}", ts=f"{index}.0") for index in range(1, 6))
    selected = select_fallback_evidence_ids(_evidence_map(*hits))
    assert len(selected) == MAX_FALLBACK_EVIDENCE_IDS
