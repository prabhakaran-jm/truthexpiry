...
def test_selector_fake_selects_fake(no_fake_env: None, fixed_clock, monkeypatch):
    monkeypatch.setenv("TRUTH_EXPIRY_CLAIM_EXTRACTOR", "fake")
...
def test_selector_live_with_openai_key_selects_live(
    no_fake_env: None, fixed_clock, monkeypatch
):
    monkeypatch.setenv("TRUTH_EXPIRY_CLAIM_EXTRACTOR", "live")
    monkeypatch.setenv("OPENAI_API_KEY", "sk-test")
    pipeline = build_pipeline(
...
def test_selector_live_without_key_fails_startup(no_fake_env: None, monkeypatch):
...
def test_blank_openai_key_fails_startup(no_fake_env: None, monkeypatch):
    monkeypatch.setenv("TRUTH_EXPIRY_CLAIM_EXTRACTOR", "live")
...
def test_invalid_selector_fails_startup(no_fake_env: None, monkeypatch):
...
def test_unset_selector_defaults_to_fake(no_fake_env: None, fixed_clock, monkeypatch):
    monkeypatch.delenv("TRUTH_EXPIRY_CLAIM_EXTRACTOR", raising=False)
    pipeline = build_pipeline(
        clock=fixed_clock,
        use_fakes=False,
        slack_client=WebClient(token="xoxb-test"),
...
def test_anthropic_key_alone_does_not_satisfy_live_mode(no_fake_env: None, monkeypatch):
    monkeypatch.setenv("TRUTH_EXPIRY_CLAIM_EXTRACTOR", "live")
    monkeypatch.delenv("OPENAI_API_KEY", raising=False)
...
def test_fake_extractor_with_live_rts_and_lifecycle(
    no_fake_env: None, fixed_clock, monkeypatch
):
    monkeypatch.setenv("TRUTH_EXPIRY_CLAIM_EXTRACTOR", "fake")
    pipeline = build_pipeline(
